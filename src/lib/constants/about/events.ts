import { Events } from "@/lib/interfaces&types/resources";

export const events: Events[] = [
  {
    id: "event-1",
    title: "Mental Health Awareness Week",
    description:
      "A week-long event dedicated to raising awareness about mental health, featuring expert talks, support groups, and community outreach.",
    date: "2025-10-10",
    location: "Los Angeles, CA",
    image: "https://plus.unsplash.com/premium_photo-1682611785720-fcb0b6a299ab",
    href: "/events/mental-health-awareness-week",
  },
  {
    id: "event-2",
    title: "Global Mental Health Summit",
    description:
      "An international event bringing together mental health professionals, activists, and those affected by mental health challenges to discuss the future of global mental health care.",
    date: "2025-09-20",
    location: "New York, NY",
    image: "https://images.unsplash.com/photo-1608308221487-02e61c839fbb",
    href: "/events/global-mental-health-summit",
  },
  {
    id: "event-3",
    title: "Youth Mental Health Forum",
    description:
      "A forum focusing on the mental health challenges faced by young people, including workshops, panel discussions, and advocacy efforts for better mental health resources.",
    date: "2025-11-15",
    location: "Chicago, IL",
    image: "https://images.unsplash.com/photo-1610190204871-e144b4f7a6c8",
    href: "/events/youth-mental-health-forum",
  },
  {
    id: "event-4",
    title: "Mental Health in the Workplace Conference",
    description:
      "A conference for HR professionals, managers, and employees to discuss strategies for creating mentally healthy workplaces and supporting employees' well-being.",
    date: "2025-12-01",
    location: "Boston, MA",
    image: "https://images.unsplash.com/photo-1606787131261-c0fa88c3ed47",
    href: "/events/mental-health-in-the-workplace-conference",
  },
  {
    id: "event-5",
    title: "World Mental Health Day Celebration",
    description:
      "A global event focused on mental health advocacy, featuring workshops, live performances, and community-driven discussions aimed at de-stigmatizing mental illness.",
    date: "2025-10-10",
    location: "Global (Virtual Event)",
    image: "https://images.unsplash.com/photo-1603265531371-e5f75a7f8ad9",
    href: "/events/world-mental-health-day-celebration",
  },
  {
    id: "event-6",
    title: "Mental Health & Technology Symposium",
    description:
      "This symposium brings together mental health professionals and tech innovators to explore how technology is transforming mental health care, including AI and telehealth solutions.",
    date: "2025-09-18",
    location: "San Francisco, CA",
    image: "https://images.unsplash.com/photo-1580138226795-27f7b255d94b",
    href: "/events/mental-health-technology-symposium",
  },
];

export const workshops: Events[] = [
  {
    id: "workshop-1",
    title: "Mindfulness for Stress Reduction",
    description:
      "A hands-on workshop teaching mindfulness techniques to help reduce stress, improve focus, and promote mental well-being.",
    date: "2025-06-15",
    location: "Los Angeles, CA",
    image: "https://plus.unsplash.com/premium_photo-1682611821813-e18f32c75cfa",
    href: "/workshops/mindfulness-for-stress-reduction",
  },
  {
    id: "workshop-2",
    title: "Cognitive Behavioral Therapy (CBT) Basics",
    description:
      "An introductory workshop to help participants understand and apply Cognitive Behavioral Therapy techniques for managing negative thoughts and behaviors.",
    date: "2025-07-10",
    location: "San Francisco, CA",
    image: "https://images.unsplash.com/photo-1604288230364-f8b402e86e9c",
    href: "/workshops/cbt-basics",
  },
  {
    id: "workshop-3",
    title: "Building Emotional Resilience",
    description:
      "A workshop aimed at helping individuals build emotional resilience to cope with life’s challenges and stressors more effectively.",
    date: "2025-08-05",
    location: "Orange, CA",
    image: "https://images.unsplash.com/photo-1608902724795-7f3b957c6b7d",
    href: "/workshops/building-emotional-resilience",
  },
  {
    id: "workshop-4",
    title: "Managing Anxiety with Breathing Techniques",
    description:
      "Learn effective breathing exercises and relaxation techniques designed to manage anxiety and improve overall mental health.",
    date: "2025-07-25",
    location: "New York, NY",
    image: "https://images.unsplash.com/photo-1587456343509-1e5e7ed69b34",
    href: "/workshops/managing-anxiety-with-breathing-techniques",
  },
  {
    id: "workshop-5",
    title: "Self-Care Strategies for Mental Health",
    description:
      "A workshop focused on the importance of self-care for mental health, including strategies for improving sleep, nutrition, and emotional balance.",
    date: "2025-06-30",
    location: "Chicago, IL",
    image: "https://images.unsplash.com/photo-1580910542063-24c0f48cc10e",
    href: "/workshops/self-care-strategies-for-mental-health",
  },
  {
    id: "workshop-6",
    title: "Overcoming Negative Thought Patterns",
    description:
      "This workshop explores techniques to identify and reframe negative thought patterns using Cognitive Behavioral Therapy and other therapeutic approaches.",
    date: "2025-08-20",
    location: "San Diego, CA",
    image: "https://images.unsplash.com/photo-1617983663669-d1e6b0419f1b",
    href: "/workshops/overcoming-negative-thought-patterns",
  },
];
