import { JournalPrompts } from "@/lib/interfaces&types/resources";
import { IoChatbubble } from "react-icons/io5";

export const gratitudeJournalingPrompts: JournalPrompts[] = [
  {
    prompt: "What are three things I am truly grateful for today?",
    ideas: [
      "Focus on both big and small things — from relationships to simple moments of joy.",
      "Think about things that bring you peace and happiness, no matter how fleeting.",
      "What are you grateful for in the present moment, right now?",
    ],
    Icon: IoChatbubble,
  },
  {
    prompt:
      "How can I practice gratitude to enhance my resilience and perspective?",
    ideas: [
      "Reflect on the role of gratitude in your life and how it can help you stay positive during tough times.",
      "What are some things you’re grateful for, even in challenging situations?",
      "How can practicing gratitude shift your perspective and help you build resilience?",
      "What daily gratitude practices can you incorporate into your routine to support your emotional well-being?",
    ],
    Icon: IoChatbubble,
  },
  {
    prompt: "Write a letter to someone who has inspired you.",
    ideas: [
      "What qualities or actions of this person have inspired you?",
      "How has this person influenced your life or perspective?",
      "What specific moments or lessons do you want to thank them for?",
      "How do you hope to carry their inspiration into your own life?",
    ],
    Icon: IoChatbubble,
  },
  {
    prompt:
      "How can I practice gratitude as a tool for emotional healing and growth?",
    ideas: [
      "Think about how gratitude has helped you shift your perspective during difficult times.",
      "What gratitude practices (e.g., journaling, expressing thanks) can you incorporate into your daily life?",
      "How can cultivating gratitude support your emotional healing journey and enhance your well-being?",
    ],
    Icon: IoChatbubble,
  },
  {
    prompt: "How has gratitude positively impacted my life so far?",
    ideas: [
      "Reflect on the moments when focusing on gratitude helped shift your perspective.",
      "How has practicing gratitude affected your relationships, health, or sense of well-being?",
      "In what ways has being grateful influenced your mindset, especially in challenging situations?",
    ],
    Icon: IoChatbubble,
  },
  {
    prompt:
      "Write about a time you felt a deep sense of gratitude. What were you grateful for, and how did it impact your perspective?",
    ideas: [
      "What specific event or experience triggered this feeling of gratitude?",
      "How did this moment change your outlook on life?",
      "What emotions did you experience during this moment of gratitude?",
      "How can you cultivate more moments of gratitude in your daily life?",
      "What role does gratitude play in your overall well-being?",
      "How can you express gratitude to others in meaningful ways?",
      "What lessons about appreciation have you learned from this experience?",
      "How can you shift your mindset to focus on gratitude more often?",
      "What practices can help you maintain a grateful heart?",
    ],
    Icon: IoChatbubble,
  },
  {
    prompt:
      "What is something I am grateful for that I often overlook or take for granted?",
    ideas: [
      "Consider the things you often overlook in your daily life (e.g., access to clean water, a safe home).",
      "Reflect on aspects of life that you might not always notice but are deeply important.",
      "Think about what you tend to take for granted and reframe it as something to be thankful for.",
    ],
    Icon: IoChatbubble,
  },
  {
    prompt: "How can I express my gratitude to someone in my life today?",
    ideas: [
      "Think about someone who has made a difference in your life recently, and express your gratitude.",
      "Consider ways you can show appreciation through words, acts of kindness, or gestures.",
      "How can you reach out to a friend, family member, or colleague to acknowledge their positive impact on you?",
    ],
    Icon: IoChatbubble,
  },
  {
    prompt:
      "How does it feel to pause and reflect on the things I am grateful for?",
    ideas: [
      "Take a few deep breaths and sit in silence as you reflect on the positive aspects of your life.",
      "Notice how gratitude shifts your mental and emotional state in the moment.",
      "What does it feel like to appreciate the present without worrying about the past or future?",
    ],
    Icon: IoChatbubble,
  },
  {
    prompt:
      "What challenges in my life have helped me grow and become stronger?",
    ideas: [
      "Think about a difficult experience and how it led to personal growth or newfound strength.",
      "How did overcoming a challenge help you appreciate the good things in your life?",
      "What lessons have you learned from difficult situations that have shaped your sense of gratitude?",
    ],
    Icon: IoChatbubble,
  },
  {
    prompt: "Who or what inspires feelings of gratitude in me, and why?",
    ideas: [
      "Think about a person, place, or thing that inspires deep appreciation.",
      "Reflect on the qualities in others that make you feel grateful to know them.",
      "What about certain experiences or environments brings gratitude to your heart?",
    ],
    Icon: IoChatbubble,
  },
  {
    prompt:
      "What am I looking forward to, and how can I approach it with a grateful heart?",
    ideas: [
      "Consider upcoming events or experiences and think about how you can embrace them with gratitude.",
      "How can you appreciate the journey and not just the destination?",
      "What are you excited about, and how can you approach it with an open, thankful mindset?",
    ],
    Icon: IoChatbubble,
  },
  {
    prompt:
      "How can I make gratitude a more consistent part of my daily routine?",
    ideas: [
      "Think about small ways to incorporate gratitude into your day (e.g., starting or ending the day with gratitude).",
      "What practices, habits, or reminders can help you stay focused on the positive?",
      "How can you share your gratitude more regularly with others?",
    ],
    Icon: IoChatbubble,
  },
  {
    prompt:
      "What is a recent moment in which I felt truly grateful, and what made it special?",
    ideas: [
      "Reflect on a recent moment where you felt a deep sense of appreciation.",
      "What about that experience or interaction made it stand out as something to be grateful for?",
      "How can you carry that sense of gratitude forward in your daily life?",
    ],
    Icon: IoChatbubble,
  },
  {
    prompt:
      "What are some things I can do to cultivate more gratitude in my life?",
    ideas: [
      "Explore actions or habits that will help you nurture a sense of gratitude, such as keeping a gratitude journal.",
      "How can you reframe negative thoughts into opportunities for gratitude?",
      "What practices or rituals (e.g., expressing thanks before meals, writing thank-you notes) can help you foster more gratitude?",
    ],
    Icon: IoChatbubble,
  },
  {
    prompt:
      "How does my current state of mind affect my ability to feel and express gratitude?",
    ideas: [
      "Notice if there are any mental or emotional blockages preventing you from feeling gratitude.",
      "How do feelings like stress, anxiety, or negativity impact your ability to appreciate the present moment?",
      "How can you shift your mindset to be more open and receptive to gratitude?",
    ],
    Icon: IoChatbubble,
  },
  {
    prompt: "What positive qualities do I appreciate most about myself?",
    ideas: [
      "Reflect on the personal strengths, values, and qualities that you hold dear.",
      "What do you admire most about yourself, and how can you show more appreciation for these traits?",
      "Consider how practicing self-gratitude contributes to your sense of well-being.",
    ],
    Icon: IoChatbubble,
  },
  {
    prompt: "How can I express more gratitude for the simple joys in life?",
    ideas: [
      "Think about everyday moments that bring you joy (e.g., a cup of tea, a walk in nature).",
      "How can you approach these moments with more awareness and appreciation?",
      "What simple pleasures have the power to brighten your day?",
    ],
    Icon: IoChatbubble,
  },
  {
    prompt:
      "How has gratitude helped me shift my perspective on challenges or setbacks?",
    ideas: [
      "Consider how gratitude has helped you reframe obstacles as opportunities for growth.",
      "How does a gratitude mindset change your approach to challenges?",
      "What can you be thankful for, even in difficult times?",
    ],
    Icon: IoChatbubble,
  },
  {
    prompt:
      "How can I use gratitude to inspire and motivate myself in my personal goals?",
    ideas: [
      "Reflect on how practicing gratitude can help you stay focused and positive as you work toward your goals.",
      "What role does gratitude play in maintaining motivation and resilience?",
      "How can you tap into the power of gratitude to fuel your personal growth and aspirations?",
    ],
    Icon: IoChatbubble,
  },
  {
    prompt:
      "What role does gratitude play in my relationships, and how can I express more of it?",
    ideas: [
      "Think about how gratitude strengthens your connections with others.",
      "How can you express appreciation to the people who support and care for you?",
      "What are some small, meaningful ways you can show gratitude in your relationships?",
    ],
    Icon: IoChatbubble,
  },
];
export const journalingPromptsgratitudeJournalingPrompts = [];
